        ...
        if (isset($_POST["submit"]))
            {
                ...
                if (!empty($tenant)&&!empty($book))
                    {
                $sql="DELETE FROM book_renting WHERE Tenant_ID_TENANT = $tenant AND Book_ID_BOOK = $book";
                $result= mysqli_query($dbConn, $sql);
                    if (!$result)
                        {
                             die ('Грешка!'. mysqli_error($dbConn));
                        }
                 echo 'Успешно върнахте книга';
                    }
         else {
            echo 'Не се получи!';    
            }
